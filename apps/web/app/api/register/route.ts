...
export async function POST(req: Request) {
  try {
    ...
    if (!email || !password || !ime || !uloga) {
      ...
      let exists = null;
    exists = await prisma.korisnik.findUnique({ where: { email } });
    if (exists) {
    ...
    return NextResponse.json({ success: true, user: { id: user.id, email: user.email, ime: user.ime, uloga: user.uloga } });
  } catch (e) {
